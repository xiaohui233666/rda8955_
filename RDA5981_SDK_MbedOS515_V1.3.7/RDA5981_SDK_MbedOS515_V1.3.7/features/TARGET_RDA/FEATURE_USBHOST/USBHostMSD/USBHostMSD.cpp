...
void USBHostMSD::init() {
    USB_INFO("usbhostmsd init begin \r\n");
    dev_connected = false;
    dev = NULL;
    bulk_in = NULL;
    bulk_out = NULL;
...
{
    USB_INFO("usbhostmsd connect begin \r\n");
    ...
    for (uint8_t i = 0; i < MAX_DEVICE_CONNECTED; i++) {
        if ((dev = host->getDevice(i)) != NULL) {
            ...
            USB_INFO("Trying to connect MSD device\r\n");
...
#if defined(TARGET_UNO_91H)
void USBHostMSD::setMsgQueue(void *queue)
{
    usb_msgQ = queue;
}
#endif
...
#if defined(TARGET_UNO_91H)
    uint32_t i = 0;
    for (uint32_t b = block_number; b < block_number + count;) {
    #if 0
        if (dataTransfer((uint8_t*)buffer, b, count, HOST_TO_DEVICE))
            return -1;
    #else
    while (dataTransfer((uint8_t*)buffer, b, count, HOST_TO_DEVICE))
       if (++i == 4)
          return -1;
    #endif
        buffer += 512*count;
...
int USBHostMSD::disk_read(uint8_t* buffer, uint32_t block_number, uint32_t count) {
    ...
    uint32_t i = 0;
    for (uint32_t b = block_number; b < block_number + count;) {
        while (dataTransfer((uint8_t*)buffer, b, count, DEVICE_TO_HOST))
            if (++i == 4)
                return -1;
