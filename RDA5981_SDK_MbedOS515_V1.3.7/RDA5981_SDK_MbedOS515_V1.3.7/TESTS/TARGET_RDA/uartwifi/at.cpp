...
{
    unsigned int index, total_len, len, tmp_len, size = 0;
    unsigned char *buf;
...
int do_wap( cmd_tbl_t *cmd, int argc, char *argv[])
{
    ...
    unsigned int ip, msk, gw, dhcps, dhcpe, flag = 0;
...
int conn(char *ssid, char *pw, char *bssid)
{
    ...
    fixip_flag = 0;
	connbefore = rda_get_cur_time_ms();
    ...
    if (ip_addr) {
		conntime = rda_get_cur_time_ms() - connbefore;
        RDA_AT_PRINT("\r\nClient IP Address is %s,conntime = %lu\r\n", ip_addr,conntime);
        conn_flag = 1;
        return 0;
    } else {
...
int do_rf_dump(cmd_tbl_t *cmd, int argc, char *argv[])
{
    int ret = 0;
    u16 reg, value;

    RESP("all rf regs are below:\r\n");

    for(reg = 0; reg < 0x180; reg++)
    {
        if(reg > 0xDF && reg < 0x131)
            continue;
        ret = wland_rf_read(reg, &value);
...
int do_phy_dump(cmd_tbl_t *cmd, int argc, char *argv[])
{
    int ret = 0;
    u32 reg, value;

    RESP("all phy regs are below:\r\n");
...
int do_mac_reg_read(cmd_tbl_t *cmd, int argc, char *argv[])
{
    int ret = 0;
    u16 reg;
    u32 value;
    ...
    if(reg > 0x707)
        return -1;
    reg -= (reg % 4);
    ret = wland_mac_reg_read(reg, &value);
...
int do_mac_reg_write(cmd_tbl_t *cmd, int argc, char *argv[])
{
    int ret = 0;
    u16 reg;
    u32 value;
    ...
    if(reg > 0x707)
        return -1;
    reg -= (reg % 4);
    ret = wland_mac_reg_write(reg, value);
...
int do_mac_reg_dump(cmd_tbl_t *cmd, int argc, char *argv[])
{
    int ret = 0;
    u16 reg;
    u32 value;

    RESP("all mac regs are below:\r\n");

    for(reg = 0; reg < 0x707; reg += 4)
    {
        ret = wland_mac_reg_read(reg, &value);
...
void add_cmd()
{
    ...
    cmd_tbl_t cmd_list[] = {
        ...
        {
            "AT+RRF",       2,   do_rf_read,
            "AT+RRF - read rf reg value: AT+RRF={rf_reg_addr(hex)}"
        },
        {
            "AT+WRF",       3,   do_rf_write,
            "AT+WRF  - write value to rf reg: AT+WRF={rf_reg_addr(hex)},{value(hex)}"
        },
        {
            "AT+DRF",       1,   do_rf_dump,
            "AT+DRF - dump all the rf reg values: AT+DRF"
        },
        /* PHY reg CMD */
        {
            "AT+RPHY",       2,   do_phy_read,
            "AT+RPHY - read phy reg value: AT+RPHY={phy_reg_addr(hex)}"
        },
        {
            "AT+WPHY",       3,   do_phy_write,
            "AT+WPHY  - write value to phy reg: AT+WPHY={phy_reg_addr(hex)},{value(hex)}"
        },
        {
            "AT+DPHY",       1,   do_phy_dump,
            "AT+DPHY - dump all the phy reg values: AT+DPHY"
        },
        /* MAC reg CMD */
        {
            "AT+RMAC",       2,   do_mac_reg_read,
            "AT+RMAC - read mac reg value: AT+RMAC={mac_reg_addr(hex)}"
        },
        {
            "AT+WMAC",       3,   do_mac_reg_write,
            "AT+WMAC  - write value to mac reg: AT+WMAC={mac_reg_addr(hex)},{value(hex)}"
        },
        {
            "AT+DMAC",       1,   do_mac_reg_dump,
            "AT+DMAC - dump all the mac reg values: AT+DMAC"
        },
    };
