...
4 对寄存器操作
4.1 RF
	AT+WRF							将值写入指定rf寄存器，地址允许输入范围（[0x0-0xDF]、[0x131-0x17F]）
		AT+WRF=0xc, 0x58			第1个参数是要写入rf寄存器的相对地址，第2个是要写入的值，两个都需要以16进制方式输入（以0x或0X开头）
	AT+RRF							读取指定rf寄存器的值
		AT+RRF=0xc					参数是读取rf寄存器的相对地址，以16进制方式输入（以0x或0X开头）
	AT+DRF							读取所有rf寄存器的值
4.2 PHY
	AT+WPHY							将值写入指定phyf寄存器，地址允许输入范围（[0x0-0xFF]）
		AT+WPHY=0xc, 0x58			第1个参数是要写入phy寄存器的相对地址，第2个是要写入的值，两个都需要以16进制方式输入（以0x或0X开头）
	AT+RPHY							读取指定phy寄存器的值
		AT+RPHY=0xc					参数是读取phy寄存器的相对地址，以16进制方式输入（以0x或0X开头）
	AT+DPHY							读取所有phy寄存器的值
4.3 MAC
	AT+WMAC							将值写入指定mac寄存器，地址允许输入范围（0x0-0x704）
		AT+WMAC=0xc, 0x58			第1个参数是要写入mac寄存器的相对地址(不用加上PA_BASE基址)。mac寄存器地址值以4字节对齐，如输入值
									非对齐，则向下取整，比如输入地址为0x7,则读取0x4的寄存器，第2个是要写入的值，两个都需要以16进制方式输入（以0x或0X开头）
	AT+RMAC							读取指定mac寄存器的值
		AT+RMAC=0xc					参数是读取mac寄存器的相对地址，以16进制方式输入（以0x或0X开头）
	AT+DMAC							读取所有mac寄存器的值
