...
/*  mp4ff_read_stsz modify by bolv  */
#if 1
static int32_t rda_mp4ff_read_stsz(mp4ff_t *f)
{
    mp4ff_read_char(f); /* version */
    mp4ff_read_int24(f); /* flags */
    f->track[f->total_tracks - 1]->stsz_sample_size = mp4ff_read_int32(f);
    ...
    if (f->track[f->total_tracks - 1]->stsz_sample_size == 0) {
        f->track[f->total_tracks - 1]->stsz_table_offset = f->current_position;
        f->current_position += f->track[f->total_tracks - 1]->stsz_sample_count * 4;//每个stsz_sample_size占用4字节
    }
...
static int32_t mp4ff_read_mvhd(mp4ff_t *f)
{
    ...
    /* preferred_rate */
    mp4ff_read_int32(f); /*mp4ff_read_fixed32(f);*/
    /* preferred_volume */
    mp4ff_read_int16(f); /*mp4ff_read_fixed16(f);*/
    for (i = 0; i < 10; i++) {
...
int32_t mp4ff_atom_read(mp4ff_t *f, const int32_t size, const uint8_t atom_type)
{
    ...
    if (atom_type == ATOM_STSZ) {
        /* sample size box */
        rda_mp4ff_read_stsz(f);//modify by bolv
