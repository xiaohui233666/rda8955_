...
static long rx_time_ms = 0;
static long long fcs_gd_sum = 0, phy_gd_sum = 0, phy_er_sum = 0, fcs_er_sum = 0;
int hut_log_handler(void *data)
{
    char *result_buf = NULL;
    long fcs_gd = 0, phy_gd = 0, phy_er = 0, fcs_er = 0;

    result_buf = (char *)data;

    rx_time_ms++;

    result_buf = strstr(result_buf, "=");
    if (result_buf == NULL) {
        mbed_error_printf("result_buf incomplete!\n");
        return -1;
    }

    fcs_gd = strtol(result_buf+2, &result_buf, 10);
    if(fcs_gd < 0) {
        mbed_error_printf("fcs_gd < 0!\n");
        return -1;
    }
    ...
    phy_gd = strtol(result_buf+2, &result_buf, 10);
    if(phy_gd < 0) {
        mbed_error_printf("phy_gd < 0!\n");
        return -1;
    }
    ...
    phy_er = strtol(result_buf+2, &result_buf, 10);
    if(phy_er < 0) {
        mbed_error_printf("phy_er < 0!\n");
        return -1;
    }
    ...
    fcs_er = strtol(result_buf+2, &result_buf, 10);
    if(fcs_er < 0) {
        mbed_error_printf("fcs_er < 0!\n");
        return -1;
    }
...
int do_get_rx_result_sum(cmd_tbl_t *cmd, int argc, char *argv[])
{
    RESP_OK_EQU("UT: fcs_gd = %lld, phy_gd = %lld, phy_er = %lld, fcs_er = %lld\r\n", fcs_gd_sum, phy_gd_sum, phy_er_sum, fcs_er_sum);
...
void add_cmd()
{
    ...
    cmd_tbl_t cmd_list[] = {
        ...
        {
            "AT+RXSUM",         1,   do_get_rx_result_sum,
            "AT+RXSUM           - get rx test result sum"
        },
        {
