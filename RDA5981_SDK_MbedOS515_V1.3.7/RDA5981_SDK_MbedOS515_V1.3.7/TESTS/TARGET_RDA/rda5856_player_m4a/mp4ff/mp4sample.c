...
int32_t rda_mp4ff_chunk_of_sample(const mp4ff_t *f, const int32_t track, const int32_t sample)
{
    int32_t total_entries = 0;
    int32_t chunk2entry;
    int32_t chunk1, chunk2, chunk1samples, range_samples, total = 0;
	int32_t chunk_sample, chunk;
    ...
    /*  add by bolv  */
    f->track[track]->stco_current_chunk = chunk;
    f->track[track]->stco_current_chunk_first_sample = chunk_sample;
    f->track[track]->stco_current_chunk_samples = f->track[track]->stsc_samples_per_chunk[chunk2entry - 1];
    M4ALOG("stco_current_chunk:%d,stco_current_chunk_first_sample:%d,stco_current_chunk_samples:%d\n", f->track[track]->stco_current_chunk,\
        f->track[track]->stco_current_chunk_first_sample, f->track[track]->stco_current_chunk_samples);
    /*  add by bolv  */
    //printf("chunk2entry:%d\n", chunk2entry);
...
int32_t rda_mp4ff_chunk_to_offset(const mp4ff_t *f, const int32_t track, const int32_t chunk)
{
    const mp4ff_track_t * p_track = f->track[track];
                                       ...
                                       const int32_t chunk_sample, const int32_t sample)
{
    ...
    else
    {
        ...
        for(i = 0, total = 0; i < (sample - chunk_sample); i++)
        {
            total += p_track->stsz_table[i];
        }
    }
...
int32_t rda_mp4ff_audio_frame_size(const mp4ff_t *f, const int32_t track, const int32_t sample)
{
    int32_t bytes;
    const mp4ff_track_t * p_track = f->track[track];
    ...
    if (p_track->stsz_sample_size)
    {
		...
		bytes = p_track->stsz_table[sample - p_track->stco_current_chunk_first_sample];
    }
...
int32_t rda_mp4ff_set_sample_position(mp4ff_t *f, const int32_t track, const int32_t sample)
{
    int32_t offset;

    //offset = mp4ff_sample_to_offset(f, track, sample);
	offset = f->track[track]->stco_current_chunk_offset + rda_mp4ff_sample_range_size (f, track, \
		f->track[track]->stco_current_chunk_first_sample, sample);
    mp4ff_set_position(f, offset);

    return 0;
}

int32_t rda_mp4ff_get_sample_position_and_size(mp4ff_t *f, const int32_t track, const int32_t sample, int32_t *offset, int32_t *size)
{
	*offset = f->track[track]->stco_current_chunk_offset + rda_mp4ff_sample_range_size (f, track, \
		f->track[track]->stco_current_chunk_first_sample, sample);
	*size = rda_mp4ff_audio_frame_size(f, track, sample);
